...
void hmac_sha1_binary(const void *key, uint16_t keylength_b, const void *msg, uint32_t msglength_b, void *dest, uint32_t *dest_len)
{
	sha1_hash_t hash;
	size_t sha1hash_max;
	size_t i;
	hmac_sha1(&hash, key, keylength_b * 8, msg, msglength_b * 8);
	sha1hash_max = sizeof(sha1_hash_t);

	for (i = 0; i < sha1hash_max; i ++)
	{
		*((unsigned char *)dest + i) = hash[i];
	}

	*dest_len = sha1hash_max;
}

void hmac_sha1_hex(void *dest, const void *key, uint16_t keylength_b, const void *msg, uint32_t msglength_b)
{
	sha1_hash_t hash;
	size_t sha1hash_max;
	size_t i;
